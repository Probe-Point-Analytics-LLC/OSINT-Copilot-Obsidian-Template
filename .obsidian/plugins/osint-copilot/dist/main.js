import { Plugin, PluginSettingTab, Setting, Notice, TFile, Modal, ItemView, MarkdownRenderer, requestUrl, } from "obsidian";
// Graph plugin imports
import { EntityType, ENTITY_CONFIGS, validateEntityName } from './src/entities/types';
...
export default class VaultAIPlugin extends Plugin {
    ...
    async onload() {
        ...
        this.graphApiService = new GraphApiService(this.settings.graphApiUrl, this.settings.reportApiKey);
        ...
        this.registerView(GRAPH_VIEW_TYPE, (leaf) => {
            ...
            return new GraphView(leaf, this.entityManager, (entityId) => this.onEntityClick(entityId), (entityId) => this.showEntityOnMap(entityId));
        });
        this.registerView(TIMELINE_VIEW_TYPE, (leaf) => new TimelineView(leaf, this.entityManager, (entityId) => this.onEntityClick(entityId)));
        this.registerView(MAP_VIEW_TYPE, (leaf) => new MapView(leaf, this.entityManager, (entityId) => this.onEntityClick(entityId)));
        ...
        const chatRibbon = this.addRibbonIcon("message-square", "OSINT Copilot Chat (Ctrl+click for new pane)", async (evt) => {
        ...
        if (this.settings.enableGraphFeatures) {
            const graphRibbon = this.addRibbonIcon("git-fork", "Entity Graph (Ctrl+click for new pane)", async (evt) => {
                const forceNew = evt.ctrlKey || evt.metaKey;
                await this.openGraphView(forceNew);
            });
            const timelineRibbon = this.addRibbonIcon("calendar", "Timeline (Ctrl+click for new pane)", async (evt) => {
                const forceNew = evt.ctrlKey || evt.metaKey;
                await this.openTimelineView(forceNew);
            });
            const mapRibbon = this.addRibbonIcon("map-pin", "Location Map (Ctrl+click for new pane)", async (evt) => {
                const forceNew = evt.ctrlKey || evt.metaKey;
    ...
    getMainEditorLeaf(viewType, forceNew) {
        ...
        const existingOsintLeaves = mainLeaves.filter(leaf => osintViewTypes.includes(leaf.view?.getViewType() || ''));
        // Find note editor leaves in main area
        const noteEditorLeaves = mainLeaves.filter(leaf => leaf.view?.getViewType() === 'markdown' || leaf.view?.getViewType() === 'empty');
    ...
    // ============================================================================
    async callRemoteModel(messages, stream = false, model) {
        if (!this.settings.reportApiKey) {
     ...
     */
    async executeStreamingFetch(endpoint, messages, onDelta) {
    ...
    async callRemoteModelStream(messages, onDelta, onRetry) {
        if (!this.settings.reportApiKey) {
            throw new Error("License key is required. Please configure it in settings.");
        }
    ...
    async askVault(query) {
        ...
        for (const note of contextNotes) {
            ...
            const excerpt = note.content.length > 1500
     ...
     */
    async askVaultStream(query, onDelta, preloadedNotes, onRetry) {
        if (!this.isAuthenticated()) {
            throw new Error("License key required for AI features. Please configure your license key in settings.");
        }
    ...
    // REPORT GENERATION
    // ============================================================================
    async generateReport(description, currentConversation, statusCallback) {
        ...
        try {
            ...
            for (let initAttempt = 1; initAttempt <= maxInitialRetries; initAttempt++) {
                try {
                    ...
                    const generateResponse = await requestUrl({
                    ...
                    if (generateResponse.status < 200 || generateResponse.status >= 300) {
                        ...
                        if (generateResponse.status === 403) {
                            ...
                            if (lowerError.includes("quota") || lowerError.includes("exhausted")) {
                                throw new Error("Companies&People quota exhausted. Please upgrade your plan or wait for quota renewal. Visit https://osint-copilot.com/dashboard/ to manage your subscription.");
                            }
                            if (lowerError.includes("expired")) {
                                throw new Error("Your license key or trial has expired. Please renew your subscription at https://osint-copilot.com/dashboard/");
                            }
                            if (lowerError.includes("inactive")) {
                                throw new Error("Your license key is inactive. Please check your account status at https://osint-copilot.com/dashboard/");
                            }
                        }
                        throw new Error(`Companies&People generation request failed (${generateResponse.status}): ${errorText.substring(0, 200)}`);
                    }
            ...
            const getPollingInterval = (elapsed) => {
                if (elapsed < 15000)
                    return 2000; // First 15s: poll every 2s (fast feedback)
                if (elapsed < 45000)
                    return 3000; // 15-45s: poll every 3s
            ...
            while (elapsedMs < maxElapsedMs && jobStatus === "processing") {
                ...
                try {
                    ...
                    if (statusData.progress) {
                        statusCallback?.(statusData.status, {
                    ...
                    if (statusData.response_ready) {
                        ...
                        if (!responseContent) {
                            try {
                                responseContent = await this.getConversationResponse(conversationId, baseUrl, reportApiKey);
                            }
                            catch (apiError) {
                                // If API endpoint doesn't exist, throw clear error
                                throw new Error(`Response is ready but content not found. ` +
                                    `Backend must return 'content' or 'response_content' in statusData when response_ready = true. ` +
                                    `Conversation ID: ${conversationId}`);
                            }
                        }
                        if (!responseContent) {
                            throw new Error(`Response is ready but no content found. ` +
                                `Backend must return 'content' or 'response_content' in statusData when response_ready = true.`);
                        }
                    ...
                    if (isNetworkError) {
                        ...
                        console.log(`[OSINT Copilot] Companies&People status poll network error (${consecutiveNetworkErrors}/${maxConsecutiveNetworkErrors}):`, `Type: ${errorType}, Message: ${errorMsg}`);
    ...
    async getConversationResponse(conversationId, baseUrl, apiKey) {
        ...
        throw new Error(`No API endpoint found to retrieve conversation messages. ` +
            `Backend must return 'content' or 'response_content' in statusData when response_ready = true.`);
    }
    // ============================================================================
     ...
     */
    async extractEntityFromQuery(query) {
        const system = "Extract the main entity mentioned in the user's query and classify it as one of: person | company | asset | event | location. Respond ONLY in JSON: {\"name\":\"<entity name or null>\",\"type\":\"person|company|asset|event|location|unknown\"}. Use English only.";
        const messages = [
        ...
        try {
            ...
            const nameVal = typeof obj?.name === "string" && obj.name.trim().length > 0
                ? obj.name.trim()
                : null;
...
class AskModal extends Modal {
    ...
    async handleAsk() {
        ...
        try {
            ...
            this.answerContainer.innerHTML = `<p style="color: var(--text-error);">Error: ${error instanceof Error ? error.message : String(error)}</p>`;
        }
    }
    onClose() {
        const { contentEl } = this;
...
class ChatView extends ItemView {
    constructor(leaf, plugin) {
        ...
        this.localSearchMode = true; // Default mode (formerly "lookup mode")
        this.darkWebMode = false;
        this.reportGenerationMode = false;
        this.osintSearchMode = false; // Leak Search mode
        // Leak Search options
        this.osintSearchOptionsVisible = false;
        this.osintSearchCountry = 'RU';
        this.osintSearchMaxProviders = 3;
        this.osintSearchParallel = true;
    ...
    async saveCurrentConversation() {
        if (!this.currentConversation) {
            this.currentConversation = await this.plugin.conversationService.createConversation(this.chatHistory.length > 0 ? this.chatHistory[0].content : undefined, this.darkWebMode, this.graphGenerationMode, this.reportGenerationMode);
        }
    ...
    render() {
        ...
        for (const option of modeOptions) {
            ...
            if (option.value === "none" && this.isGraphOnlyMode())
                optEl.selected = true;
     ...
     */
    renderOSINTSearchOptions(container) {
        ...
        countrySelect.addEventListener("change", () => {
            this.osintSearchCountry = countrySelect.value;
        });
        // Max providers
    ...
    async renameConversation(id, currentTitle) {
        new RenameConversationModal(this.app, currentTitle, async (newTitle) => {
    ...
    async renderMessages() {
        ...
        for (let i = 0; i < this.chatHistory.length; i++) {
            ...
            const messageDiv = this.messagesContainer.createDiv(`vault-ai-chat-message vault-ai-chat-${item.role}`);
            ...
            await MarkdownRenderer.render(this.app, item.content, contentDiv, "", // sourcePath - empty string for non-file content
    ...
    updateProgressBar(messageIndex, progress, intermediateResults) {
        ...
        const messageDiv = this.messagesContainer.querySelector(`.vault-ai-chat-message[data-message-index="${messageIndex}"]`);
        if (!messageDiv)
            return;
    ...
    async handleGraphOnlyMode(inputText) {
        ...
        try {
            ...
            const result = await this.plugin.graphApiService.processText(inputText, existingEntities, undefined, onRetry);
    ...
    async handleNormalChat(query) {
        ...
        const getLastAssistantContentEl = () => {
            const els = this.messagesContainer.querySelectorAll(".vault-ai-chat-message.vault-ai-chat-assistant .vault-ai-chat-content");
            if (els.length === 0)
                return null;
        ...
        try {
            ...
            const entityMsg = extracted.type === "unknown"
            ...
            const { fullAnswer, notes: finalNotes } = await this.plugin.askVaultStream(searchTerm, (delta) => {
                streamed += delta;
    ...
    async processGraphGeneration(assistantIndex, aiResponse, originalQuery, currentContent) {
        ...
        try {
            ...
            const result = await this.plugin.graphApiService.processText(textToProcess, existingEntities, undefined, onRetry);
    ...
    async handleReportGeneration(description) {
        ...
        try {
            ...
            const reportData = await this.plugin.generateReport(description, this.currentConversation, (status, progress, intermediateResults) => {
            ...
            // Save to vault
            const fileName = await this.plugin.saveReportToVault(reportData.content, description, reportData.filename);
            ...
            let finalContent = `📄 **Companies&People Generated Successfully!**\n\n` +
                `**Request:** ${description}\n\n` +
                `**Saved to:** \`${fileName}\`\n\n` +
                `---\n\n` +
                reportData.content;
    ...
    async handleOSINTSearch(query) {
        ...
        try {
            ...
            const result = await this.plugin.graphApiService.aiSearch(searchRequest, onRetry);
                ...
                if (response.status < 200 || response.status >= 300) {
                    ...
                    if (response.status === 403) {
                        ...
                        if (lowerError.includes("quota") || lowerError.includes("exceeded")) {
                            throw new Error("Investigation quota exhausted. Please upgrade your plan or wait for quota renewal. Visit https://osint-copilot.com/dashboard/ to manage your subscription.");
                        }
                        if (lowerError.includes("expired")) {
                            throw new Error("Your license key or trial has expired. Please renew your subscription at https://osint-copilot.com/dashboard/");
                        }
    ...
    pollDarkWebStatus(jobId, messageIndex, query) {
        ...
        const getPollingInterval = (elapsed) => {
            if (elapsed < 20000)
                return 3000; // First 20s: poll every 3s (fast feedback)
            if (elapsed < 60000)
                return 5000; // 20s-60s: poll every 5s
        ...
        const poll = async () => {
            ...
            try {
                ...
                const errorMsg = error instanceof Error ? error.message : String(error);
                console.error(`[OSINT Copilot] Dark web status poll error (${consecutiveErrors}/${maxConsecutiveErrors}):`, `Type: ${errorType}, Message: ${errorMsg}`);
