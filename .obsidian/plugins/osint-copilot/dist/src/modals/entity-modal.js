...
import { EntityType, ENTITY_CONFIGS, COMMON_PROPERTIES, getFTMEntityConfig, getAvailableFTMEntityTypes, getAvailableFTMIntervalTypes, validateEntityName, getEntityIcon } from '../entities/types';
...
export class EntityCreationModal extends Modal {
    ...
    async handleGeocode() {
        ...
        try {
            const result = await this.geocodingService.geocodeAddressWithRetry(address, city, state, country, (attempt, maxAttempts, delaySeconds) => {
    ...
    async handleCreate() {
        ...
        try {
            ...
            if (this.entityId) {
                ...
                entity = await this.entityManager.createEntity(this.entityType, this.properties, { skipAutoGeocode: true });
    ...
    async autoGeocodeIfNeeded() {
        ...
        try {
            const result = await this.geocodingService.geocodeAddressWithRetry(this.properties.address, this.properties.city, this.properties.state, this.properties.country, (attempt, maxAttempts, delaySeconds) => {
                this.setGeocodeStatus('loading', `Network error, retrying in ${delaySeconds}s... (attempt ${attempt}/${maxAttempts})`);
            });
...
export class EntityTypeSelectorModal extends Modal {
    constructor(app, entityManager, onEntityCreated, filterTypes) {
        super(app);
        this.entityManager = entityManager;
        this.onEntityCreated = onEntityCreated || null;
    ...
    onOpen() {
        ...
        for (const type of typesToShow) {
            ...
            typeBtn.onclick = () => {
                this.close();
                const createModal = new EntityCreationModal(this.app, this.entityManager, type, this.onEntityCreated || undefined);
                createModal.open();
            };
        }
        // Cancel button
...
export class ConnectionCreationModal extends Modal {
    ...
    async handleCreate() {
        ...
        try {
            const connection = await this.entityManager.createConnection(this.sourceEntityId, this.targetEntityId, this.relationship);
            if (connection) {
...
export class ConnectionQuickModal extends Modal {
    constructor(app, entityManager, sourceEntityId, targetEntityId, sourceLabel, targetLabel, onConnectionCreated) {
        super(app);
        this.relationship = '';
        this.properties = {};
    ...
    renderPropertyFields(intervalType) {
    ...
    createPropertyField(propertyName, propDef, isRequired) {
        if (!this.propertiesContainer)
            return;
    ...
    async handleCreate() {
        ...
        try {
            const connection = await this.entityManager.createConnection(this.sourceEntityId, this.targetEntityId, this.relationship, this.properties);
            if (connection) {
...
export class EntityEditModal extends Modal {
    ...
    onOpen() {
        ...
        contentEl.empty();
        contentEl.addClass('graph_copilot-entity-modal');
        const config = ENTITY_CONFIGS[this.entity.type];
        // Title
    ...
    createPropertyField(container, propertyName, isRequired) {
        ...
        if (propertyName === 'notes' || propertyName === 'description' || propertyName === 'text') {
            ...
            if (currentValue) {
                input.value = currentValue.replace(' ', 'T');
            }
        }
        else if (propertyName === 'latitude' || propertyName === 'longitude') {
            // Number input for coordinates
...
export class FTMEntityCreationModal extends Modal {
    ...
    onOpen() {
        ...
        const featuredNonRequired = config.featuredProperties.filter(p => !config.requiredProperties.includes(p));
    ...
    async handleCreate() {
        ...
        try {
            ...
            // Skip auto-geocoding for manual creation - user should click the Geocode button explicitly
            const entity = await this.entityManager.createFTMEntity(this.schemaName, this.properties, { skipAutoGeocode: true });
...
export class FTMEntityTypeSelectorModal extends Modal {
    ...
    filterEntityTypes() {
        ...
        const filtered = this.entityTypes.filter(type => type.label.toLowerCase().includes(query) ||
    ...
    renderEntityTypes(types) {
        ...
        for (const typeInfo of types) {
            ...
            typeBtn.onclick = () => {
                this.close();
                const createModal = new FTMEntityCreationModal(this.app, this.entityManager, typeInfo.name, this.onEntityCreated || undefined);
                createModal.open();
            };
        }
    }
...
export class FTMEntityEditModal extends Modal {
    ...
    async handleGeocode(button) {
        ...
        try {
            ...
            const result = await this.geocodingService.geocodeAddressWithRetry(address, city, state, country, (attempt, maxAttempts, delaySeconds) => {
                this.updateGeocodeStatus(`⚠️ Network error, retrying in ${delaySeconds}s... (attempt ${attempt}/${maxAttempts})`, 'warning');
            });
            ...
            // Update status
            this.updateGeocodeStatus(`✓ Geocoded: ${result.displayName}\nLat: ${result.latitude.toFixed(6)}, Lng: ${result.longitude.toFixed(6)}\nConfidence: ${result.confidence}`, 'success');
            // Re-enable button
...
export class FTMIntervalTypeSelectorModal extends Modal {
    ...
    filterIntervalTypes() {
        const query = this.searchInput?.value.toLowerCase() || '';
        const filtered = this.intervalTypes.filter(type => type.label.toLowerCase().includes(query) ||
            type.description.toLowerCase().includes(query) ||
            type.name.toLowerCase().includes(query));
    ...
    renderIntervalTypes(types) {
        ...
        for (const typeInfo of types) {
            ...
            typeBtn.onclick = () => {
                ...
                const createModal = new FTMIntervalCreationModal(this.app, this.entityManager, typeInfo.name, this.onConnectionCreated || undefined, this.sourceEntityId || undefined, this.targetEntityId || undefined);
                createModal.open();
            };
        }
    }
    ...
    async handleCreate() {
        ...
        try {
            ...
            if (entityProps.length >= 2) {
                ...
                const connection = await this.entityManager.createConnection(sourceId, targetId, this.intervalType);
                if (connection) {
