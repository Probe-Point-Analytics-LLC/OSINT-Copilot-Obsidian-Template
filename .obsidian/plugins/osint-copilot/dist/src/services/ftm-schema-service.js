...
class FTMSchemaServiceClass {
    ...
    resolveSchema(schemaName) {
        ...
        const optionalProperties = Object.keys(allProperties).filter(prop => !defaultProperties.includes(prop) && !allProperties[prop].hidden);
        const resolved = {
            name: schemaName,
    ...
    getIntervalSchemas() {
        this.initialize();
        return Array.from(this.resolvedSchemas.values()).filter(schema => !schema.abstract && this.extendsInterval(schema));
    }
    /**
    ...
    extendsInterval(schema) {
        if (schema.name === 'Interval')
            return false; // Interval itself is abstract
