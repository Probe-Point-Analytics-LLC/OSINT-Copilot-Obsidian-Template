...
import { EntityType, ENTITY_CONFIGS, getEntityLabel, generateId, sanitizeFilename, COMMON_PROPERTIES, getFTMEntityConfig } from '../entities/types';
...
export class EntityManager {
    ...
    async createFTMEntity(schemaName, properties, options) {
        ...
        const entity = {
            id,
            type: schemaName, // Use schema name as type
            label,
            properties,
    ...
    buildFTMPropertiesSection(entity, schemaName) {
        ...
        for (const [prop, value] of Object.entries(entity.properties)) {
            ...
            if (value === undefined || value === null || value === '')
                continue;
            if (prop === 'notes')
                continue; // Notes shown separately
            const propDef = config.propertyDefinitions[prop];
            const label = propDef?.label || prop;
            lines.push(`- **${label}**: ${value}`);
        }
...
`;
    ...
    getConnectionsForEntity(entityId) {
        return Array.from(this.connections.values()).filter(c => c.fromEntityId === entityId || c.toEntityId === entityId);
    }
    /**
