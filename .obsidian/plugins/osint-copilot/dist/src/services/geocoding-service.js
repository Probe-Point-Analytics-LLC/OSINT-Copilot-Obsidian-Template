 ...
 */
export var GeocodingErrorType;
(function (GeocodingErrorType) {
    GeocodingErrorType["NetworkError"] = "NETWORK_ERROR";
    GeocodingErrorType["NotFound"] = "NOT_FOUND";
    GeocodingErrorType["RateLimited"] = "RATE_LIMITED";
    GeocodingErrorType["InvalidInput"] = "INVALID_INPUT";
    GeocodingErrorType["Unknown"] = "UNKNOWN";
})(GeocodingErrorType || (GeocodingErrorType = {}));
/**
...
export class GeocodingService {
    ...
    async geocodeAddressWithRetry(address, city, state, country, onRetry) {
        ...
        for (let attempt = 0; attempt < GeocodingService.MAX_RETRIES; attempt++) {
            try {
                ...
                if (!(error instanceof GeocodingError)) {
                    ...
                    lastError = new GeocodingError(GeocodingErrorType.Unknown, error instanceof Error ? error.message : String(error));
                }
                else {
                    lastError = error;
                }
        ...
        throw new GeocodingError(GeocodingErrorType.NetworkError, `Failed to geocode after ${GeocodingService.MAX_RETRIES} attempts. Please check your internet connection.`);
    }
    /**
    ...
    async geocodeAddressWithFallback(address, city, state, country) {
        ...
        for (const query of fallbackQueries) {
            try {
                ...
                const result = await this.geocodeAddress(query.components[0], query.components[1], query.components[2], query.components[3]);
        ...
        throw lastError || new GeocodingError(GeocodingErrorType.NotFound, 'Address not found. Please check the address or enter coordinates manually.');
    }
    /**
    ...
    async geocodeAddress(address, city, state, country) {
        ...
        if (queryParts.length === 0) {
            throw new GeocodingError(GeocodingErrorType.InvalidInput, 'Please provide at least an address, city, or country');
        }
        ...
        try {
            ...
            if (response.status === 429) {
                throw new GeocodingError(GeocodingErrorType.RateLimited, 'Too many requests. Please wait a moment and try again.');
            }
            // Handle other errors
            if (response.status !== 200) {
                throw new GeocodingError(GeocodingErrorType.NetworkError, `Geocoding request failed with status ${response.status}`);
            }
            const results = response.json;
            if (!results || results.length === 0) {
                throw new GeocodingError(GeocodingErrorType.NotFound, 'Address not found. Please check the address or enter coordinates manually.');
            }
            ...
            throw new GeocodingError(GeocodingErrorType.NetworkError, 'Failed to connect to geocoding service. Please check your internet connection.');
        }
    }
    /**
...
GeocodingService.NOMINATIM_URL = 'https://nominatim.openstreetmap.org/search';
GeocodingService.USER_AGENT = 'OSINTCopilot-Obsidian-Plugin/1.0 (https://github.com/Probe-Point-Analytics-LLC/OSINT-Copilot-plugin)';
GeocodingService.REQUEST_TIMEOUT = 10000; // 10 seconds
GeocodingService.MIN_REQUEST_INTERVAL = 1100; // 1.1 seconds between requests
// Retry configuration
GeocodingService.MAX_RETRIES = 5;
GeocodingService.INITIAL_RETRY_DELAY = 1000; // 1 second
