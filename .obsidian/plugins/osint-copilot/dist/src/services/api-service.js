...
export class GraphApiService {
    ...
    async checkHealth() {
        try {
            ...
            const rootResponse = await requestUrl({
                url: `${this.baseUrl}/`,
                method: 'GET',
                headers: this.getHeaders(),
                throw: false
     ...
     */
    async processText(text, existingEntities, referenceTime, onRetry) {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                const response = await this.fetchWithTimeout(`${this.baseUrl}/api/process-text`, {
                    method: 'POST',
                    headers: this.getHeaders(),
                    mode: 'cors',
                    credentials: 'omit',
     ...
     */
    async aiSearch(request, onRetry) {
        // First, try to connect if we haven't checked yet
        if (!this.isOnline) {
            await this.checkHealth();
        }
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                const response = await this.fetchWithTimeout(`${this.baseUrl}/api/bot-aggregator/ai-search`, {
                    method: 'POST',
                    headers: this.getHeaders(),
                    mode: 'cors',
                    credentials: 'omit',
