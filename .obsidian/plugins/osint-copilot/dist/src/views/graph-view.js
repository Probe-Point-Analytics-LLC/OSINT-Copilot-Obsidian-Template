...
import { ENTITY_CONFIGS, EntityType, getEntityIcon } from '../entities/types';
import { ConnectionCreationModal, ConnectionQuickModal, FTMEntityTypeSelectorModal, FTMEntityEditModal, ConnectionEditModal } from '../modals/entity-modal';
import { GraphHistoryManager, HistoryOperationType } from '../services/graph-history-manager';
...
export class GraphView extends ItemView {
    constructor(leaf, entityManager, onEntityClick, onShowOnMap) {
         ...
         */
        this.savePositionsDebounced = this.debounce(() => this.savePositions(), 1000);
    ...
    handleConnectionModeClick(nodeId, nodeLabel) {
        if (!this.sourceNodeId) {
            ...
            const modal = new ConnectionQuickModal(this.app, this.entityManager, this.sourceNodeId, nodeId, this.sourceNodeLabel || '', nodeLabel, (connectionId) => {
     ...
     */
    openConnectionModal() {
        const modal = new ConnectionCreationModal(this.app, this.entityManager, (connectionId) => {
            // Record connection creation in history and add edge incrementally
            if (connectionId) {
                const connection = this.entityManager.getConnection(connectionId);
                if (connection) {
     ...
     */
    openEntityCreator() {
        const modal = new FTMEntityTypeSelectorModal(this.app, this.entityManager, (entityId) => {
    ...
    showNodeContextMenu(event, entityId, entityType, label) {
        ...
        const editItem = this.createMenuItem('✏️ Edit Entity', () => {
            ...
            if (entity) {
                ...
                const editModal = new FTMEntityEditModal(this.app, this.entityManager, entity, () => {
    ...
    showEdgeContextMenu(event, connectionId, relationship, sourceId, targetId) {
        ...
        const editItem = this.createMenuItem('✏️ Edit Connection', () => {
            ...
            if (connection) {
                ...
                const editModal = new ConnectionEditModal(this.app, this.entityManager, connection, () => {
                    // Record edit in history
    ...
    async loadSavedPositions() {
        try {
            ...
            if (file instanceof TFile) {
                ...
                console.log(`[GraphView] Loaded ${this.nodePositionsCache.size} saved node positions:`, Array.from(this.nodePositionsCache.entries()).slice(0, 3));
            }
            else {
    ...
    addEntityToGraphPreserveLayout(entity) {
        ...
        if (existingNode.length > 0)
            return;
        const config = ENTITY_CONFIGS[entity.type] || ENTITY_CONFIGS[EntityType.Person];
        // Ensure label is a string
        const entityLabel = entity.label != null ? String(entity.label) : '';
